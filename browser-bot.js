    ...
    async start() {
        ...
        if (config.splashMode) {
            ...
            if (config.twocaptcha.enabled && config.captchaExpected) {
                this.captchaSolution =
                    await solveCaptchas(this.page, await findCaptchas(this.page));
            }
...
async function setListeners(page) {
...
async function lauchHeadedBrowser(cookies) {
...
// returns boolean
function solveCaptchas(page, captchas) {
    return new Promise(async function (resolve, reject) {
...
async function cartProduct(captchaSolution, page, instance) {
